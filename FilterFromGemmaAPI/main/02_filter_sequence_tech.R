...
micr_GSEs <- API_output %>%
  ...
  dplyr::filter(API_output$technologyType == "ONECOLOR" | API_output$technologyType == "TWOCOLOR")
...
two_col_GSEs <- micr_GSEs %>%
  dplyr::filter(micr_GSEs$technologyType == "TWOCOLOR")
...
write_delim(data.frame(affymetrix_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","affymetrix_GSEids"), delim = "\n", col_names = FALSE)
write_delim(data.frame(ion_torrent_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","ion_torrent_experiments"), delim = "\n", col_names = FALSE)
write_delim(data.frame(nano_string_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","nano_string_experiments"), delim = "\n", col_names = FALSE)
write_delim(data.frame(lnc_rna_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","lnc_rna_experiments"), delim = "\n", col_names = FALSE)
write_delim(data.frame(ab_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","ab_experiments"), delim = "\n", col_names = FALSE)
write_delim(data.frame(pacbio_experiments$shortName), file = paste0(results_dir,"/", "filter_by_technology", "/","pacbio_experiments"), delim = "\n", col_names = FALSE)
